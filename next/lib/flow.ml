  ...
  let formula =
    Formula.init_tl ?name (fun _tl _i period ->
        List.fold_left
          (fun acc (src_p, v) ->
            let ov_start =
              Date.max (Period.start_date src_p) (Period.start_date period)
            in
            let ov_end =
              Date.min (Period.end_date src_p) (Period.end_date period)
            in
...
let convert ~rate s =
  { formula = Formula.convert ~rate s.formula; hint = H_scale (rate, s.hint) }
  ...
  let rec accrue_via_ctx ctx ~start_date ~end_date =
    ...
    | Q_source_periods { pairs; split_fn } ->
        List.fold_left
          (fun acc (src_p, v) ->
            let ov_start =
              Date.max (Period.start_date src_p) start_date
            in
...
let rec hint_to_ctx = function
  ...
  | H_source_periods { pairs; split_fn } ->
      Materialized.Q_source_periods { pairs; split_fn }
