...
(** {b Internal} -- Declarative computation DAG over timelines.
    ...
    The core computation engine. {!Flow} and {!Balance} wrap this module with provenance tracking
    for partial-period accrual and date-based interpolation. Not exposed to library users (private
    module via dune).

    {b Thread safety.} Formula values are pure DAG descriptions, but evaluation mutates internal
    memoization caches. Do not evaluate the same formula concurrently from multiple domains. *)

type +'c t
(** The type for formulas. A node in a DAG that is evaluated against a {!Timeline.t} to produce one
    [float] per period. Covariant in ['c] (the currency/unit phantom tag). *)
...
(** [init_flow f] is a formula that produces [f period] at each period. Wrapped by {!Flow.init}. *)

val init : ?name:string -> (int -> Period.t -> float) -> 'c t
(** [init f] is a formula that produces [f i period] at period [i]. Wrapped by {!Flow.init_indexed}.
*)

val init_tl : ?name:string -> (Timeline.t -> int -> Period.t -> float) -> 'c t
(** [init_tl f] is like {!init} but [f] also receives the {!Timeline.t}. Use when the computation
    needs timeline-level information (e.g. total number of periods). *)
...
val growth_simple :
  ...
  float ->
  'c t
(** [growth_simple ~start_date ~rate initial] is simple (linear) growth. [daycount] defaults to
    {!Daycount.actual_360}. *)
...
val growth_compound :
  ...
  float ->
  'c t
(** [growth_compound ~start_date ~rate initial] is compound growth. [daycount] defaults to
    {!Daycount.actual_360}. *)

val year_frac : ?name:string -> (Date.t -> Date.t -> float) -> 'c t
(** [year_frac daycount] produces [daycount start end] for each period. *)
...
(** [of_events events] bins [(date, value)] pairs into periods. Multiple events in the same period
    are summed. Periods with no events produce [0.0].
...
(** [named name s] attaches [name] for {!Cycle_error}, {!Convergence_error} diagnostics and
    {!Deps.pp_dot} labels. Shares memoization state with [s]. *)
...
(** [clamp ~lo ~hi s] clamps each value to \[[lo]; [hi]\]. *)

val round : int -> 'c t -> 'c t
(** [round digits s] rounds each value to [digits] decimal places. *)

val sum : ?name:string -> 'c t list -> 'c t
(** [sum ss] is the pointwise sum. The empty list produces [0.0]. *)
...
(** {1 Cross-period operators}
    ...
    The only way to express temporal relationships. Same-period cycles are detected at evaluation
    time ({!Cycle_error}). Cross-period cycles are structurally impossible: {!prev} shifts by one
    period and {!scan} only reads its own earlier output. *)

val prev : ?name:string -> 'c t -> default:float -> 'c t
(** [prev s ~default] produces [default] at period 0 and [s.(i-1)] at period [i > 0]. The primitive
    for cross-period dependencies. *)
...
(** [scan ~init f flow] is a running accumulation:
    ...
    [acc] is the output of [scan] itself at the prior period, not the input [flow]. *)

val cumsum : ?name:string -> init:float -> 'c t -> 'c t
(** [cumsum ~init flow] is [scan ~init (fun ~acc ~x -> acc +. x) flow]. *)

val feedback : ?name:string -> default:float -> ('c t -> 'c t * 'a) -> 'a
(** [feedback ~default f] ties a self-referential knot via {!prev}. Calls [f] with a formula
    representing the {e previous period's} value ([default] at period 0). [f] returns
    [(definition, result)] where [definition] is the formula fed back.
...
(** [fixpoint ~guess f] iterates within each period to find [x] such that [f (const x)] converges to
    [x]. At period 0 iteration starts from [guess]; subsequent periods warm-start from the prior
    converged value.
...
(** [convert ~rate s] scales [s] by [rate] and changes the currency tag. *)
...
(** Raised when {!fixpoint} does not converge. [iterations] is the number attempted. *)

val eval : Timeline.t -> 'c t -> float array
(** [eval tl s] materializes [s] against [tl], returning one [float] per period. Each (formula,
    period) cell is computed at most once.

    Raises {!Cycle_error} or {!Convergence_error}. *)

val eval_many : Timeline.t -> 'c t list -> float array list
(** [eval_many tl ss] materializes each formula, sharing a single memoization context to avoid
    redundant computation of shared subexpressions.
...
(** [eval_materialized tl s] is like {!eval} but returns a {!Materialized.t} with the timeline bound
    to the values.
...
(** {1 Date queries}
    ...
    Low-level timeline probing used by {!Flow} and {!Balance} for provenance-aware accrual and
    interpolation. *)

module Query : sig
  type split_fn =
    start_date:Date.t -> end_date:Date.t -> split_date:Date.t -> value:float -> float * float
  (** The type for functions that split a period's value at a date. Returns [(before, after)] where
      [before +. after = value]. *)
  ...
  (** [interpolate tl values date] is the portion of the enclosing period's value that falls before
      [date]. [split_fn] defaults to {!default_split_fn}.
  ...
  (** [accrue tl values ~start_date ~end_date] sums values over the date range. Boundary periods are
      split: first period contributes its {e after} portion, last period its {e before} portion.
      Returns [0.0] if the range does not overlap the timeline. *)

  val balance_at :
    ?split_fn:split_fn -> Timeline.t -> balance:float array -> flow:float array -> Date.t -> float
  (** [balance_at tl ~balance ~flow date] is the interpolated balance at [date]: the prior period's
      ending balance plus the portion of the current period's flow that falls before [date]. Used by
...
module Deps : sig
  ...
  (** A node in the computation DAG. [kind] is one of: ["const"], ["init"], ["map"], ["map2"],
