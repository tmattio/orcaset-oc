...
(** {b Internal} -- Point-in-time quantities.
    ...
    Wraps {!Formula.t} with provenance tracking for date-based interpolation via {!Materialized.at}.
    Public API is constrained by [orcaset2.mli]. *)

type 'c t
(** The type for balances. Pairs a {!Formula.t} with provenance metadata for {!Materialized.at}
    interpolation. *)
...
(** [of_dates observations] distributes date-keyed observations into periods using
    last-observation-wins semantics: each period takes the value of the most recent observation on
    or before the period's end date. Periods before any observation produce [before_first] (default
    [0.0]).
...
(** [init f] is a balance that produces [f period] at each period. Low-level; prefer {!const} or
    {!of_array}. *)
...
(** [roll_forward ~init flow] is a balance where:
    ...
    Stores the companion flow for provenance-aware {!Materialized.at} interpolation. *)

val roll_forward_with :
  ?name:string -> init:float -> (acc:float -> x:float -> float) -> 'c Flow.t -> 'c t
(** [roll_forward_with ~init f flow] is like {!roll_forward} with a custom accumulation function [f]
    instead of addition. *)
...
(** [prev b ~default] shifts by one period. Implementation primitive for {!at_period_start};
    delegates to {!Flow.prev}. *)

(** {1 Bridge to Flow} *)

val sample : ?name:string -> 'c t -> 'c Flow.t
(** [sample b] reads the end-of-period balance at each period as a flow. The resulting flow has
    {!Flow.Materialized.Q_cell_based} provenance. *)

val change : 'c t -> default:float -> 'c Flow.t
(** [change b ~default] is the per-period delta of [b]: at period 0 the change is [b.(0) - default],
    at period [i > 0] the change is [b.(i) - b.(i-1)]. *)

(** {1 Feedback and fixpoint} *)

val feedback : ?name:string -> default:float -> ('c t -> 'c t * 'a) -> 'a
(** [feedback ~default f] ties a self-referential knot. Calls [f] with a balance representing the
    {e previous period's} value ([default] at period 0). [f] returns [(definition, result)] where
    [definition] is the balance fed back.

    Raises {!Flow.Cycle_error} if the definition contains a same-period cycle. *)

val fixpoint : ?name:string -> ?tol:float -> ?max_iter:int -> guess:float -> ('c t -> 'c t) -> 'c t
(** [fixpoint ~guess f] iterates within each period to find convergence.

    [tol] defaults to [1e-10]. [max_iter] defaults to [100].

    Raises {!Flow.Convergence_error} if [max_iter] iterations are exhausted. *)
...
module Materialized : sig
  type 'c t
  (** The type for materialized balance results. Each value is bound to its period, with provenance
      for {!at}. *)

  val make : Timeline.t -> float array -> 'c t
  (** [make tl values] binds [values] to [tl] without provenance. {!at} falls back to end-of-period.
  *)
  ...
  (** [at m date] is the interpolated balance at [date].
      ...
      When provenance is available (from {!roll_forward}, {!of_dates}, or composed via
      {!add}/{!sub}/etc.), computes the exact value by accruing the companion flow up to [date].

      When provenance is unavailable (cell-local combinators or {!make}), returns the end-of-period
      value for the enclosing period.
...
(** [eval tl b] materializes [b] against [tl]. When [b] was created with {!roll_forward}, the
    companion flow is co-evaluated for provenance-aware {!Materialized.at}.

    Raises {!Flow.Cycle_error} or {!Flow.Convergence_error}. *)

val eval_values : Timeline.t -> 'c t -> float array
(** [eval_values tl b] is like {!eval} but returns the raw array without provenance. *)
...
(** [unsafe_to_formula b] is the underlying formula. Provenance is lost. *)
...
(** [unsafe_of_array arr] wraps a raw array without provenance. The array is captured by reference
